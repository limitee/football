...
impl SenderHandler {
     ...
     */
    pub fn js_api(&self, req: Request, res: Response, path:&str) {
        let js_path = &path[7..];
    ...
    pub fn data_api_err(&self, err_code:i32, head:Json, mut res: Response) {
        let mut back_json = json!("{}");
       	json_set!(&mut back_json; "err"; CONS.get_err(err_code));
	    let back_msg_obj = self.api.back_err(&head, back_json.to_string());
        let back_str = back_msg_obj.to_string();
        {
            let mut headers = res.headers_mut();
            headers.set(
    ...
    pub fn data_api_success(&self, mut body:Json, 
        head:Json, uuid:&str, key:String, mut res: Response) 
    {
        json_set!(&mut body; "uuid"; uuid);
       	json_set!(&mut body; "err"; CONS.get_err(ErrCode::Success as i32));
       	let back_msg_obj = self.api.back(head, key, body.to_string());
        let back_str = back_msg_obj.to_string();
        {
            let mut headers = res.headers_mut();
            headers.set(
     ...
     */
    pub fn data_api(&self, mut req: Request, res: Response, _path:&str) {
        ...
        let rst = self.api.check(&(self.db), &req_map);
        if let Err(err_code) = rst {
            self.data_api_err(err_code, head, res); 
            return;
        }
        let msg = rst.unwrap();

        //check the uuid node
        let uuid_node_op = msg.find_path(&["body", "uuid"]); 
        if uuid_node_op.is_none() {
            self.data_api_err(ErrCode::KeyIsMissing as i32, head, res); 
            return;
        }

        let uuid_node = uuid_node_op.unwrap();
        let uuid = uuid_node.as_string().unwrap();
        ...
        let body = rst.unwrap();
        self.data_api_success(body, head, uuid, key, res);
    }
