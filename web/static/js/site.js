...
juicer.register('getLocDateStr', function(sec){
    return CurSite.getDateStr(sec*1000);
});

//全局定时器
CurSite.jobs = {};
CurSite.job_count = 0;
CurSite.job_stick_count = 0;

CurSite.add_job = function(count, handler) {
    CurSite.job_count++;
    var id = CurSite.job_count;
    CurSite.jobs[id] = {stick:count, handler:handler};
    return id;
}

CurSite.remove_job = function(id) {
    CurSite.jobs[id] = undefined;
    delete CurSite.jobs[id];
}

CurSite.job_handler = function() {
    CurSite.job_stick_count++;
    for(var key in CurSite.jobs) {
        var job = CurSite.jobs[key];
        if(CurSite.job_stick_count%job.stick === 0) {
            job.handler();
        }
    }
    setTimeout(CurSite.job_handler, 1000);
}
