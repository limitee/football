...
var Com = function(cfg, cb) {
    ...
    self.parent = $("#" + cfg.id);
    var loading_text = '<p class="bg-info">系统正在努力加载中...</p>';
    self.parent.html(loading_text);
...
Com.prototype.destroy = function() {
    var self = this;
    for(var id in self.cr) {
        self.cr[id].destroy();
    }
    if(self.ins && self.ins.destroy) {
        self.ins.destroy();
    }
}
