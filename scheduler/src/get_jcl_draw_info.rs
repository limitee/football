...
pub fn handle_jcl_by_date(date:&DateTime<Local>, db:&DataBase<MyDbPool>) -> Result<i32, i32> {
    let mut page = 0;
    loop {
...
fn handle_map(map:&BTreeMap<i64, String>, db:&DataBase<MyDbPool>) -> Result<i32, i32> {
    for (key, value) in map {
        info!("{}-{}", key, value);
        let sql = format!("update term set draw_number='{}',version=version+1 where game_id=301 and code={} and status=50", value, key);
...
fn get_jcl_draw_info(date:&DateTime<Local>, page:i64) -> Result<(i64, BTreeMap<i64, String>), String> {
    let url = "http://info.sporttery.cn/basketball/match_result.php"; 
    ...
    */
    ...
    for set in result_vec {
        ...
        if set[11] == "\u{5df2}\u{5b8c}\u{6210}" && set[12] == "\u{8be6}\u{7ec6}" {
            map.insert(term_code, set[10].replace(":", ","));
        }
    }
